...
from imports import *
from systems.trigonometric import trigonometric
from systems.two_mass import two_mass
from qLPV_MPC import qLPV_MPC
...
sim_parameters['W_params'] = RCI_concur['W']
sim_parameters['u_scaler'] = dataset['u_scaler']
sim_parameters['y_scaler'] = dataset['y_scaler']
sim_parameters['HU'] = dataset['constraints']['HU']
sim_parameters['hU'] = RCI_LTI['hU_modified']
sim_parameters['HY'] = dataset['constraints']['HY']
sim_parameters['hY'] = dataset['constraints']['hY']
sim_parameters['only_px'] = identification_data['only_px']
sim_parameters['kappa_safety'] = identification_data['kappa']
...
ny = model.ny
nu = model.nu
RCI_dict = identification_data['RCI_concur']
F = RCI_dict['F']
V = RCI_dict['V']
yRCI = np.array(RCI_dict['yRCI'])
...
for k in range(m_bar):
    ...
    xRCI_vert_LPV_init[k] = V[k] @ RCI_LPV_init['yRCI']
    yRCI_vert[k] = C @ V[k] @ yRCI
    yRCI_vert_LTI[k] = C_LTI @ V[k] @ yRCI_LTI
    yRCI_vert_LPV_init[k] = C_LPV_init @ V[k] @ RCI_LPV_init['yRCI']
...
Y_RCI_set_model = polytope.qhull(yRCI_vert)
Y_RCI_H = Y_RCI_set_model.A * model.y_scaler[1]
Y_RCI_h = Y_RCI_set_model.b + Y_RCI_H @ model.y_scaler[0]
Y_RCI_set = polytope.Polytope(Y_RCI_H, Y_RCI_h)

Y_RCI_set_LTI_model = polytope.qhull(yRCI_vert_LTI)
Y_RCI_H_LTI = Y_RCI_set_LTI_model.A * model.y_scaler[1]
Y_RCI_h_LTI = Y_RCI_set_LTI_model.b + Y_RCI_H_LTI @ model.y_scaler[0]
Y_RCI_set_LTI = polytope.Polytope(Y_RCI_H_LTI, Y_RCI_h_LTI)

Y_RCI_set_LPV_init_model = polytope.qhull(yRCI_vert_LPV_init)
...
Y_RCI_set_LPV_init = polytope.Polytope(Y_RCI_H_LPV_init, Y_RCI_h_LPV_init)

Hcon_plant = HY * model.y_scaler[1]
hcon_plant = hY + Hcon_plant @ model.y_scaler[0]
Hcon = polytope.Polytope(Hcon_plant, hcon_plant)
Y_con_vert_plant = polytope.extreme(Hcon)
...
#Disturbance bounds
hW = np.array([model.W_params[0]+model.W_params[1], model.W_params[0]-model.W_params[1]]).reshape(1,-1)[0]
w_bounds_ub = model.y_scaler[0] + (model.W_params[0]+model.W_params[1])/model.y_scaler[1]
w_bounds_lb = model.y_scaler[0] + (model.W_params[0]-model.W_params[1])/model.y_scaler[1]
...
polytope.Polytope(F, yRCI).project([1,2]).plot(ax = ax2, color='r',alpha=0.1)
polytope.Polytope(F, yRCI_LPV_init).project([1,2]).plot(ax = ax2, color='g',alpha=0.1)
polytope.Polytope(F, yRCI_LTI).project([1,2]).plot(ax = ax2, color='b',alpha=0.1)
ax2.autoscale()

if ny>1:
    Hcon.project([1,2]).plot(ax = ax4, color='g',alpha=0.1)
    Y_RCI_set.project([1,2]).plot(ax = ax4, color='r',alpha=0.1)
    Y_RCI_set_LTI.project([1,2]).plot(ax = ax4, color='b',alpha=0.1)
    Y_RCI_set_LPV_init.project([1,2]).plot(ax = ax4, color='g',alpha=0.1)
    ax4.autoscale()
...
for t in range(N_sim):
    ...
    Q_track = block_diag(np.eye(model.nx), 0.1)
...
if 1:
    ...
    polytope.Polytope(F,yRCI).project([1,2]).plot(ax=ax3, color='w', alpha=1, linewidth = 1, linestyle = '-', edgecolor = 'k')
    polytope.Polytope(F,yRCI_LPV_init).project([1,2]).plot(ax=ax3, color='b', alpha=0.5, linewidth = 1, linestyle = '-', edgecolor = 'b')
    polytope.Polytope(F,yRCI_LTI).project([1,2]).plot(ax=ax3, color='r', alpha=0.5, linewidth = 1, linestyle = '-', edgecolor = 'r')
